...
// your application must make use of a Graph data structure
// (building off of this GraphADT.java interface) in a way
// that makes appropriate use of Dijkstra's shortest path algorithm.

/**
 * This ADT represents a directed graph data structure with only positive edge
 * weights.  Duplicate node values are not allowed.
 *
 * @param <T> the data type stored at each graph node
 * @param <E> the numeric data type stored at each graph edge, with a
 *            doubleValue() method that always returns a value >=0.0
 */
public interface GraphADT<T, E extends Number> {
     ...
     */
    public boolean insertEdge(T pred, T succ, E weight);
     ...
     */
    public boolean removeEdge(T pred, T succ);
     ...
     */
    public boolean containsEdge(T pred, T succ);
     ...
     */
    public E getEdge(T pred, T succ);
     ...
     */
    public List<T> shortestPathData(T start, T end);
     ...
     */
    public double shortestPathCost(T start, T end);
