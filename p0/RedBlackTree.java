...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
     ...
     * this tree. We use two pointers to represent the nodes in this method, x and
     * y.
     * 
     * @param x the node to rotate
     */
    private void leftRotate(Node<T> x) {
        Node<T> y = x.context[2]; // y = the right child of x
        x.context[2] = y.context[1]; // set x to the left child of y
        if (y.context[1] != null) { // if y has a left child
            y.context[1].context[0] = x; // set the parent of y's left child to x
        }
        y.context[0] = x.context[0]; // set y's parent to x's parent
        if (x.context[0] == null) { // if x is the root
            root = y; // set y to the root
        } else if (x == x.context[0].context[1]) { // if x is the left child of its parent
            x.context[0].context[1] = y; // set y to the left child of x's parent
        } else { // if x is the right child of its parent
            x.context[0].context[2] = y; // set y to the right child of x's parent
        }
        y.context[1] = x; // set y to the left child of x
        x.context[0] = y; // set x to the parent of y
    }
    ...
    public void rightRotate(Node<T> x) { // the same as leftRotate, but with left and right
        ...
        if (y.context[2] != null) {
            y.context[2].context[0] = x;
        }
        y.context[0] = x.context[0];
        if (x.context[0] == null) {
            root = y;
        } else if (x == x.context[0].context[2]) {
            x.context[0].context[2] = y;
        } else {
            x.context[0].context[1] = y;
        }
        y.context[2] = x;
        x.context[0] = y;
    }
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        // also, check if the provided nodes are even in this tree
